      ...
      code = """
        ...
        const connectionQueries = connectionsList
            .map((conn) => {
                ...
                return `SELECT *, ${escapedRawSlug} AS "Connection_Profile_URL_Input" FROM x6_148 WHERE user_id = ${escapedUserId} AND "Connection_Profile_URL" IN (${escapedSlugs.join(', ')})`;
            })
            .filter((query) => typeof query === 'string' && query.length > 0);
        
        if (connectionQueries.length === 0) {
            return "SELECT * FROM x6_148 WHERE 1=0;";
        }
