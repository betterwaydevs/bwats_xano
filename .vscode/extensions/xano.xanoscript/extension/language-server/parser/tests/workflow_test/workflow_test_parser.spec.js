...
describe("Xanoscript WorkflowTest Parser", () => {
  it("parses all the valid sources", () => {
    const errors = [];
    const files = readdirSync(
      join(dirname("./"), "parser/tests/workflow_test/valid_sources")
    );
    files.forEach((file) => {
      const source = readFileSync(
        join(dirname("./"), "parser/tests/workflow_test/valid_sources", file),
        "utf8"
      );
        ...
        parser.errors.forEach((error) => {
          errors.push({
            previous: error.previousToken
              ...
              : "N/A",
            file: `parser/tests/api_group/valid_sources/${file}:${error.token.startLine}:${error.token.startColumn}`,
